		...
		<div className="mb-24 mt-4 lg:m-0">
			...
			<form ref={form} action={formAction} className="grid gap-4 mt-4">
				<div className="relative">
					...
					<label htmlFor="name" className="absolute text-sm text-neutral-400 duration-300 transform -translate-y-4 scale-75 top-4 z-10 origin-[0] start-2.5 peer-focus:text-secondary-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-4 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto">{t('name')}</label>
				</div>
				...
				<div className="relative">
					...
					<label htmlFor="email" className="absolute text-sm text-neutral-400 duration-300 transform -translate-y-4 scale-75 top-4 z-10 origin-[0] start-2.5 peer-focus:text-secondary-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-4">{t('email')}</label>
				</div>

				<NewsletterButton />

				{formState.error && formState.error.length > 0 && <p className="text-red-500">{formState.error}</p>}
				{formState.error === null && <p className="text-green-500">{t('thanks')}</p>}
			</form>
		</div>
	)
}