			...
			<div className="mt-8 grid gap-6 lg:grid-cols-3">
				{
					data.map(post => (
						<article key={post.id} className="flex flex-col gap-3">
							<Link href={`/blog/post/${post.id}`}><Image width={2400} height={1600} className="hover:brightness-75 duration-300 rounded h-full w-full object-cover object-center aspect-[3/2]" src={post.img_url} alt={post.title} /></Link>
							<div className="text-sm text-neutral-600 font-medium">{t('by')} <Link href={`/blog/user/${post.user_id}`} className="text-neutral-950 font-semibold hover:text-secondary-700 duration-300">{post.full_name}</Link> <span className="hidden lg:inline">•</span><br className="lg:hidden" /> {formatDate(post.created_at, locale)}</div>
