		...
		<section className="bg-[#0a4a3a] text-tertiary-50 relative overflow-hidden">
			...
			<div className="container mx-auto px-4 lg:px-16 py-12 lg:py-16 relative z-10">
				...
				<div className="max-w-4xl">
					...
					<div className="mt-8 lg:col-span-2 grid gap-12 lg:grid-cols-2">

						{
							data.map((post) => (
								<article key={post.id} className="flex flex-col gap-3 min-w-96 shrink-0">
									<Link href={`/blog/post/${post.id}`}><img className="shadow-md shadow-primary-950/30 hover:brightness-75 duration-300 rounded h-full w-full object-cover object-center aspect-[3/2]" src={post.img_url} alt={post.title} /></Link>
									...
									<Link className="hover:text-neutral-300 duration-300" href={`/blog/post/${post.id}`}><h3 className={`font-source font-semibold text-2xl tracking-tighter leading-tight`}>{post.title}</h3></Link>
